  ...
  <div>
    <b-modal id="modal-center-add-role" centered hide-footer hide-header>
      <div class="card-body">
        ...
        <div class="card-text d-flex mt-3">
          <button class="btn btn-primary-outlined ms-auto" @click="$bvModal.hide('modal-center-add-role')">Cancel</button>
          <button @click="addRole" class="btn btn-primary ms-2">Add</button>
        </div>
      </div>
    </b-modal>
    ...
    <b-modal id="modal-center-delete-role" centered hide-footer hide-header>
      <div class="card-body">
        ...
        <div class="card-text d-flex mt-3">
          <button @click="$bvModal.hide('modal-center-delete-role')" class="btn btn-primary-outlined ms-auto">Cancel</button>
    ...
    <div class="view-all-table-wrapper">
      ...
      <div class="nexcard">
        ...
        <div class="d-flex mb-4">
          ...
          <div class="ms-auto">
            <button v-b-modal.modal-center-add-role class="btn btn-primary">Add Role</button>
          </div>
        </div>

        <div class="nextable">
          <div class="row nextable__head g-0">
            <div class="col"><p>Role Name</p></div>
            <div class="col"><p>Description</p></div>
            <div class="col action"><p>Action</p></div>
          </div>

          <div class="nextable__body__collection">
            <div class="row nextable__body g-0" v-for="(item, index) in roles" :key="'employees' + index">
              <div class="col">
                <p>{{ item.name }}</p>
              </div>
              <div class="col">
              ...
              <div class="col action">
                <div class="action-menu">
                  ...
                  <div class="action-menu-dropdown">
                    <span @click="edit(item.id)" class="edit"><span class="material-icons-outlined">edit</span>Edit</span>
                    <span @click="deleteRole(item.id)" class="delete"><span class="material-icons-outlined">delete</span>Delete</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  methods: {
    ...
    getActivities() {
      ...
      if (this.search_word == null) {
        ...
        while (this.filterrole.length > 0) {
          this.filterrole.pop();
        }
        ...
        this.allroles.forEach((element) => {
          if (element.name.toLowerCase().search(this.search_word) >= 0) {
            ...
            this.filterrole.push(element);
          }
        });

        this.roles = this.filterrole;
        // //
      }
    },

    addRole() {
      Settings.addRole({ name: this.name, description: this.description })
        .then((response) => {
    ...
    deleteValue() {
      Settings.removeRole(this.deleteId)
        .then((response) => {
          this.$toast.success(response.data.message);
    ...
    edit(data) {
      console.log(data);
      this.$router.push({ name: "edit_role", params: { id: data } });
    },
  },
