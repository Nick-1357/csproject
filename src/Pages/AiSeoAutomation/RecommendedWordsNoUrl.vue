      ...
      <div class="recommendedwords__table">
        ...
        <div class="row recommendedwords__tbody g-0" v-for="(word, index) in words" :key="'words' + index">
          <div class="col-3 d-flex align-items-center">
            <span class="ms-3 recommendedwords__words">{{ word.words }}</span>
          </div>
          ...
          <div class="col d-flex align-items-center">
            ...
            <div class="pin__bar__graph">
              ...
              <div class="progress">
                ...
                <div div class="progress-bar bg-warning" role="progressbar" :style="{ width: +((word.avg / word.highest) * 100) + '%' }" aria-valuenow="75" aria-valuemin="0" aria-valuemax="100"></div>
              </div>
              ...
              <div
                class="value blue"
                :style="{
                  left: 'calc(' + (word.count / word.highest) * 100 + '% - 9px) !important',
                }"
              >
              ...
              <div
                class="value red"
                :style="{
                  left: 'calc(' + (word.avg / word.highest) * 100 + '% - 9px) !important',
                }"
              >
                <span>{{ word.avg }}</span>
              </div>
  ...
  created() {
    ...
    if (this.content.url.length != 0) {
      ...
      this.recommend.forEach((recommend, index_recommend) => {
        ...
        for (const [key, value] of Object.entries(recommend)) {
          if (key != "avg" && key != "words") {
            websites.push({ key: key, value: value });
          }
        }

        this.words.push({
          avg: recommend.avg,
          words: recommend.words,
          websites: websites,
        });
      });
