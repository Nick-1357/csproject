    ...
    <!-- <b-modal id="modal-confirm-content planner" centered hide-footer hide-header>
      <div class="card-body">
        ...
        <div class="card-text d-flex mt-3">
          ...
          <button @click.once="confirmNextStep" class="btn btn-primary ms-2 text-white">Confirm</button>
        </div>
      </div>
    </b-modal> -->
      ...
      <div class="row">
        <div class="col">
          <div class="d-flex align-items-center">
            <span class="fw-600 mb-1">Choose your Subtopics</span>
          </div>

          <!-- Suggestions Tab -->

          <div class="selectcontent__content">
            <div class="selectcontent__checkbox" v-for="(normal, index_normal) in subtopics" :key="'topic' + index_normal">
              <input :name="'topic' + index_normal" :id="'topic' + index_normal" type="checkbox" :value="normal" v-model="chosenSubtopic" />
...
export default {
  ...
  methods: {
    changetoEditorTitle(index) {
      ...
      this.subtopics[index].splice(index, 1, arr);
    ...
    RegenerateContent(sub, index) {
      ...
      this.subtopics[index].content = "";
      this.subtopics[index].regenerate = true;
      this.subtopics[index].loading = true;
      this.subtopics[index].new = true;
      ...
      Automation.contentPlanner(this.$route.params.id, obj)
        .then((response) => {
          ...
          this.subtopics[index].content = response.data.data.generated_text;

          this.subtopics[index].loading = false;
          this.subtopics[index].new = true;
    ...
    nextStep() {
      //   console.log(this.chosenSubtopic);

      var arr = JSON.parse(localStorage.content_planner_final_content);

      arr.subtopics = this.chosenSubtopic;
  ...
  created() {
    JSON.parse(localStorage.content_planner_subtopic).forEach((element, index) => {
      this.subtopics.push({
        title: element,
        content: "",
        regenerate: false,
    ...
    if (JSON.parse(localStorage.content_planner_planners).length != 0) {
      JSON.parse(localStorage.content_planner_planners).forEach((element) => {
        this.subtopics.forEach((element_subtopic) => {
    ...
    if (localStorage.content_planner_final_content != undefined && localStorage.content_planner_final_content != "undefined") {
      if (JSON.parse(localStorage.content_planner_final_content).subtopics.length != 0) {
        JSON.parse(localStorage.content_planner_final_content).subtopics.forEach((element) => {
          this.chosenSubtopic.push(element);

          console.log(element.title);

          if (!this.subtopics.find((element_h1) => element_h1.title === element.title)) {
            this.subtopics.push(element);
          }
        });
      } else {
        this.chosenSubtopic.push(this.subtopics[0]);
      }
    } else {
      this.chosenSubtopic.push(this.subtopics[0]);
    }
