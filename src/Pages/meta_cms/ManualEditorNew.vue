      ...
      <div class="d-flex">
        <div class="d-flex w-100">
          <div class="w-60 d-flex">
            <div class="w-100">
              <select id="inputGroupSelect04" aria-label="Example select with button addon" v-model="campaign.typesFull" class="custom-select form-select rounded w-100">
            ...
            <div class="ms-2 w-100">
              <b-form-input type="text" autocorrect="off" autocomplete="off" id="uri" name="uri" v-model="campaign.url" />
            </div>
          </div>
          <div class="w-40 d-flex">
            <div class="ms-auto">
              <button @click="save_changes" class="ms-2 btn btn-primary">Submit</button>
            </div>
            <div class="ms-2">
              ...
              <tbody>
                <tr :key="index" v-for="(item, index) in selected.h1">
                  ...
                  <td class="w-5 text-center">
                    <button v-if="isRemove || selected.h1" @click="removeH1Arr(index)" class="">
...
export default {
  ...
  methods: {
    ...
    page() {
      ...
      // this.campaign.url = url.pathname.substring(1);
    },
    appendTable() {
    ...
    save_changes() {
      ...
      const obj = {
        ...
        type: this.campaign.typesFull.name,
      ...
      meta_cms
        .updateClientRecommendation(obj)
        .then((response) => {
          //
  ...
  created() {
    meta_cms
      ...
      .then((response) => {
        ...
        meta_cms
          .fetchTypes(this.$route.params.url_id)
          .then((response) => {
            // //
        ...
        const foundTopics = this.campaign.elements.find((element) => element.key === "title");

        if (foundTopics != undefined) {
          this.selected.title = foundTopics.value;
        }

        const foundDesc = this.campaign.elements.find((element) => element.key === "description");
        ...
        this.campaign.elements.forEach((element) => {
          if (element.key == "h1") {
