<template>
  <div class="mt-4">
    <div class="d-flex mb-3">
      <b-button-group class="ms-auto b-button-group__custom">
        <b-button v-for="(btn, idx) in buttons" :key="idx" :pressed.sync="btn.state" @click="changeView(idx)" variant="outline-primary">
          {{ btn.caption }}
        </b-button>
      </b-button-group>
    </div>

    <div v-if="chosenView == 'Desktop'">
    ...
    <mobile v-if="chosenView == 'Mobile'"></mobile>
  </div>
</template>
  ...
  methods: {
    ...
    loadResult() {
      ...
      this.values.forEach((element) => {
        if (element.slug === "desktop_FailedAudits") {
          this.FailedAudits = JSON.parse(element.data);
        }
        if (element.slug === "desktop_DiagnosticAudits") {
          this.DiagnosticAudits = JSON.parse(element.data);
        }
        if (element.slug === "desktop_PassedAudits") {
...
.b-button-group__custom {
  height: 38px;
}
.b-button-group__custom button {
  border-radius: 20px;
}
.b-button-group__custom .btn {
  border: 1px solid #2c8de0;
  color: #2c8de0;
}
.b-button-group__custom .btn:hover {
  background-color: #dcefff;
}
.b-button-group__custom .btn.active {
  background-color: #2c8de0;
  color: #fff;
}
